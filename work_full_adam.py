...
def main(args):
    ...
    model = AttenAlex(train_data.cls_count, atten_open=not no_atten).to(device)
    criterion = nn.CrossEntropyLoss(weight=torch.tensor(cls_weight)).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=0.1)

    # read pretrained weight if necessary
    if pretrain_dir:
        load_pretrain_both_branch(model, pretrain_dir, device)
    ...
    log_dir  = log_root + '/{}'.format(log_folder)
    writer = SummaryWriter(log_dir)  # TensorboardX: https://zhuanlan.zhihu.com/p/35675109
    print('logging at:')
    print(log_dir)
