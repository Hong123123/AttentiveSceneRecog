...
# hyperparameters
log_root = config.log_root  # '/mnt/old_hexin/log_atten'
epochs = 100
pretrain_dir = ''  # config.places_alex
ckpt_dir = '/mnt/old_hexin/log/2019-04-26_20:42:46/checkpoints/best/epochs:92.ckpt'
eval_inteval_epoch = 1
save_inteval_epoch = 1 * eval_inteval_epoch
log_folder = str(datetime.now().strftime('%Y-%m-%d_%H:%M:%S'))
lr = 1e-3
...
for epoch in range(epochs):
    ...
    for batch in iter(train_loader):
        ...
        writer.add_scalar('Train/Running_Accu(steps)', float(running_acc)/batch_size, abs_step + 1)
